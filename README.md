...
![Detail](https://github.com/MoonGwangHee/MoonGwangHee/assets/94667528/ad987820-2cc2-46b0-b409-aa0c16547ab8)

<br>

1. 게임 기능 <br>

- 프레임 회피 - 순간적인 Layer 변환을 이용해 구현
- 히트박스 - 2d Capsule Collider를 이용해 구현
- 플레이어 추적 AI - Sprite Renderer 중심간의 x좌표 차이를 이용해 구현
- 랜덤패턴 - random 함수를 이용해 구현
- 피격 이벤트 - animation event와 2dCollision이벤트를 이용해 구현
<br> <br>

2. 게임 내 구조물 <br>

- GameManager - GameManager.cs를 포함하는 오브젝트. 씬 이동, 버튼, 일시정지 및 게임의 전반적인 시스템을 위해 사용.
- MapSize&Center - 스크립트 내에서 지정된 변수. 카메라가 이동할 수 있는 한계선을 정하기 위해 사용.
- StartBtn - 시작버튼. Button을 사용했으며 클릭 시 씬 전환.
- PauseBtn - 일시정지 버튼. 볼륨조절 PopUp을 활성화시키며 게임내 시간을 정지시킴.
- Warrior_Runner - Warrior Sprite를 포함하는 오브젝트. Player가 화면 밖에서 달려오는 연출을 구현하기 위해 사용. Player와 x좌표가 같아지면 소멸. 접촉시 Player z좌표를 수정해 인게임에서 보이게 함.
- Player - 물리엔진과 Warrior Sprite를 포함하고 있는 오브젝트. 플레이어가 컨트롤하게 되는 오브젝트. 시작시에는 BG 뒤에 가려져있음.
- Enemy - 물리엔진과 AI, DeathBringer Sprite를 포함하고 있는 오브젝트. Hp값이 0이되면 게임이 클리어됨.
- Hand - 접촉판정과 DeathBringer Sprite를 포함하고 있는 오브젝트. Enemy의 스킬 발동을 위해 사용되며, 평소에는 비활성화 되어있다가 스킬 발동시 그 시점 플레이어 바로 위 좌표에 활성화.
- Slime - 슬라임 Sprite와 AI를 포함하고 있는 프리펩. Enemy 스킬 사용시 인게임 내 특정 좌표에서 생성되어 낙하.
- SpawnSpot - 슬라임 스폰 장소
- Walls - 플레이어의 맵 이탈을 방지하는 오브젝트. 눈에 보이지 않고, 물리엔진이 적용되어 있지만 좌표값, 회전이 고정되어 벽 역할을 함.
- TileMap - 충돌판정을 지니고 있는 타일맵. 게임에서의 지면 역할을 함.
- BG - 배경화면 이미지.
